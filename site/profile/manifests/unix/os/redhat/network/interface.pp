define profile::unix::os::redhat::network::interface (
  $dhcp                  = undef,
  $hwaddr                = undef,
  $hotplug               = true,
  $ipaddress             = undef,
  ...
  if $hwaddr {
    if ! is_mac_address($hwaddr) {
      fail("profile::unix::network::redhat::interface::hwaddr '$hwaddr' must be a MAC address: interface '${_interface}'")
    }
    $_hwaddr = $hwaddr
  }
  else {
  ...
  if ! is_ip_address($_ipaddr) {
    fail('profile::unix::network::redhat::interface::ipaddress must be an IP address')
  }
  $_netmask = $netmask
  if ! is_ip_address($_netmask) {
    fail('profile::unix::network::redhat::interface::netmask must be an IP address')
  }
  $_onboot  = $onboot
  ...
  if $routes {
    ...
    create_resources('profile::unix::network::redhat::route', $routes, { interface => $_interface } )
  }
}
